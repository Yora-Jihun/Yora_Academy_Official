    ...
    public function submit(): void
    {
        ...
        if ($user) {
            ...
            if ($this->avatar) {
                ...
                Storage::disk('avatars')->put($filename, $this->avatar->get());
                $data['avatar'] = $filename;
            }

            $user->update($data);
            $this->dispatch('profile-updated', avatar: $user->avatar ? Storage::disk('avatars')->url($user->avatar) : asset('assets/images/Jerome_Edica.jpg'));
            $this->oldAvatar = $user->avatar;
            $this->avatar = null;
        }
